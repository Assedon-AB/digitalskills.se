...
const Toplist = ({ data, title, category, industry }: ToplistProps) => {
	...
	function getShownBranschData() {
		let showData = industry.prediction_percentages.month_6;
		switch (showMode) {
			case "Prognos 6 mån": {
	...
	function sortBy(arr: DigspecData[], mode: string, showWhat: string) {
		...
		if (preProp) {
			newData = arr.sort(
				(a: any, b: any) => b[preProp][prop] - a[preProp][prop]
			);
		} else {
	...
	function setRedirect(category: string) {
		...
		return (
			<Link href={href}>
				<a
					tabIndex={0}
					className="uppercase font-medium text-gray-500 focus:ring hover:text-blue-800"
				>
					{"se alla " +
								...
								<tr>
									...
									>
										<div className="flex items-center">
											...
											<InfoPopover
												isSmall={true}
												title={`Vad betyder ${showMode}?`}
												text={modeToText[showMode]}
												customTranslate="-translate-x-3/4"
											/>
										</div>
									</th>
								</tr>
