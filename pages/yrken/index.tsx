...
const OccupationsOverview: NextPage<OccupationPageProps> = ({
	...
	const buildCompareComponent = () => {
		return (
			<>
				<h1 className="pt-16 pl-2 text-xl">Valda yrken</h1>
				<CompareTable
	...
	const createCompareObjects = () => {
		...
		for (const dIndex in compareList) {
			...
			const index = occupations.findIndex((x) => x._id === dId);
		...
		<div className=" bg-[#fafafa] w-full h-full min-h-screen py-8">
			...
			<article className="max-w-6xl px-4 mx-auto pt-8">
								...
								<div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
									...
									<div className="mt-2">
										<p className="text-sm text-gray-500">
											...
											jämföra max 5 yrken. För att rensa
											jämförelsetabellen, klicka rensa
											under Jämför på huvudtabellen.
										</p>
									</div>
				...
				{compareList.length > 0 ? (
					...
					<CompareMissingInfo category="yrken" />
				)}
